...
def normalize_day(X_day: np.ndarray, norm) -> np.ndarray:
    ...
    CHUNK=500 window ~ 120 MB per chunk — tidak pernah OOM.
    """
    if norm is None:
        return X_day
    ...
    for start in range(0, n_win, CHUNK):
        ...
        # nan_to_num in-place — tidak buat array bool besar
...
def predict_one_day(X_day: np.ndarray, models: dict) -> dict:
    scores = {}

    if models.get("LSTM"):
        try:
            from shms_phase3a_lstm import predict as pred
            res = pred(X_day[:, :, :N_MAIN],
    ...
    if models.get("GNN"):
        try:
            ...
            scores["GNN"] = res["scores"].astype(np.float32)
        except Exception as e:
            scores["GNN"] = None
...
def run_streaming(processed_dir, cutoff_day, target_dates, norm, models):
    """Proses per hari, akumulasi skor tanpa muat semua ke RAM."""
    ...
    for day_obj in sorted(target_dates):
        ...
        X_day     = np.load(x_path)
        n_win_day = len(X_day)
        ram_gb    = X_day.nbytes / 1024**3

        print(f"  [{date_str}] y={label} ({lbl_str}) | "
              f"{n_win_day:,} win | {ram_gb:.2f} GB RAM", end=" | ")

        # Normalisasi chunked
        X_norm = normalize_day(X_day, norm)
        del X_day
        gc.collect()

        # Predict
        scores_day = predict_one_day(X_norm, models)
        del X_norm
        gc.collect()
...
def compute_metrics(all_scores, y_true, models):
    ...
    for name, sc in all_scores.items():
        ...
        # Gunakan P95 dari seluruh skor sebagai threshold operasional
        thr_norm = float(np.percentile(sc_v, 95))
...
Contoh:
  ...
  python shms_ood_validation_nov2025.py --no-normalize
    ...
    for _, r in metrics_df.iterrows():
        ...
        f1v = float(r["F1"])
        if f1v >= 0.80:
            interp = "SANGAT BAIK — deteksi in-service anomaly akurat"
        elif f1v >= 0.60:
            interp = "BAIK — mayoritas anomali terdeteksi"
        elif f1v >= 0.40:
            interp = "CUKUP — threshold perlu fine-tuning"
        else:
            interp = "PERLU REVIEW"
        print(f"  {r['Model']:<10}: F1={f1v:.3f}, AUC={r['AUC_ROC']} → {interp}")
