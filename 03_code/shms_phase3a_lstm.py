...
HP = {
    ...
    "hidden_size"   : 64,     # ukuran hidden state LSTM
    "num_layers"    : 2,      # jumlah layer LSTM (encoder & decoder)
    "dropout"       : 0.2,    # dropout untuk regularisasi

    # Training
    "batch_size"    : 64,     # windows per batch (hemat RAM)
    "learning_rate" : 1e-3,   # Adam optimizer
    "n_epochs"      : 50,     # maksimum epoch
    "patience"      : 7,      # early stopping: berhenti jika val loss
                               # tidak membaik setelah N epoch
    "clip_grad"     : 1.0,    # gradient clipping (stabilisasi training)

    # Threshold
    "threshold_pct" : 95,     # persentil RE pada data validation
...
def train(hp: dict = HP, save_dir: Path = MODEL_DIR,
    ...
    # Load data (hanya normal untuk training)
    print("\n[DATA]")
    X_train, _ = load_split_numpy("train", normal_only=True)
    X_val,   _ = load_split_numpy("val",   normal_only=True)
    ...
    # Training loop
    print(f"\n[TRAINING] {len(X_train):,} train | {len(X_val):,} val windows")
...
def calibrate_threshold(model, hp: dict = HP,
    ...
    X_val, y_val = load_split_numpy("val", normal_only=False)
    val_ds = TensorDataset(torch.FloatTensor(X_val))
    val_dl = DataLoader(val_ds, batch_size=hp["batch_size"],
                        shuffle=False, num_workers=0)
...
def evaluate(model=None, threshold: float = None,
    ...
    X_test, y_test = load_split_numpy("test", normal_only=False)
